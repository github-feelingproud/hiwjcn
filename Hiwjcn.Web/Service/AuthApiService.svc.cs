...
namespace Hiwjcn.Web.Service
{
    ...
    public class AuthApiService : IAuthApiService
    {
        ...
        {
            return await AppContext.ScopeAsync(async s => await s.Resolve_<IAuthApi>().GetAccessTokenAsync(client_id, client_secret, code, grant_type));
        }

        public async Task<_<string>> GetAuthCodeByOneTimeCode(string client_id, string scopeJson, string phone, string sms)
        {
            return await AppContext.ScopeAsync(async s => await s.Resolve_<IAuthApi>().GetAuthCodeByOneTimeCodeAsync(client_id, scopeJson, phone, sms));
        }

        public async Task<_<string>> GetAuthCodeByPassword(string client_id, string scopeJson, string username, string password)
        {
            return await AppContext.ScopeAsync(async s => await s.Resolve_<IAuthApi>().GetAuthCodeByPasswordAsync(client_id, scopeJson, username, password));
        }

        public async Task<_<LoginUserInfo>> GetLoginUserInfoByToken(string client_id, string access_token)
        {
            return await AppContext.ScopeAsync(async s => await s.Resolve_<IAuthApi>().GetLoginUserInfoByTokenAsync(client_id, access_token));
        }
    }
}
