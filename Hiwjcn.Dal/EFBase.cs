...
namespace Dal
{
    ...
    public class EFRepository1<T> : IRepository<T> where T : class, IDBTable
    {
        ...
        {
            if (!ValidateHelper.IsPlumpList(models)) { throw new Exception("参数为空"); }
            using (var db = EFManager.GetDbContext())
            {
                var set = db.Set<T>();
                foreach (var m in models)
                {
        ...
        {
            EFManager.PrepareSession(callback);
        }
        ...
        {
            EFManager.PrepareConnection(callback);
        }
        ...
        {
            EFManager.PrepareConnection(callback, isoLevel);
        }
        ...
        {
            EFManager.PrepareIQueryable<T>(callback, track);
        }
