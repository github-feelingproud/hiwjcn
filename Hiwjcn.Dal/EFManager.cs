...
namespace Hiwjcn.Dal
{
    ...
    public static class EFManager1
    {
        ...
        {
            return AppContext.GetObject<DbContext>();
        }
        ...
        public static void PrepareSession(Func<DbContext, bool> callback)
        {
            using (var db = GetDbContext())
            {
                //执行回调
        ...
        public static void PrepareIQueryable<T>(Func<IQueryable<T>, bool> callback, bool track = true)
            where T : class, IDBTable
        {
            EFManager.PrepareSession(session =>
            {
                //用来查询，所以不要跟踪实体状态
                //NopCommerce:
        ...
        /// <typeparam name="T"></typeparam>
        /// <param name="callback"></param>
        public static void PrepareConnection(Func<IDbConnection, IDbTransaction, bool> callback, IsolationLevel? isoLevel = null)
        {
