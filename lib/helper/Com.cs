...
namespace Lib.helper
{
    ...
    public static class Com
    {
        ...
        public static string BaiduTranslate(string q, string from, string to)
        {
            ...
            var salt = GetRandomNumString(10);
            ...
            var urlparam = $"{url}?{DictToUrlParams(dict)}";

            HttpClientHelper.SendHttpRequest(urlparam, null, null, null, RequestMethodEnum.GET, 20, (res) =>
            {
                if (res.IsSuccessStatusCode)
                {
                    var json = AsyncHelper.RunSync(() => res.Content.ReadAsStringAsync());
                    var data = JsonHelper.JsonToEntity<BaiduTransJson>(json);
                    trans = data?.trans_result?[0].dst;
                }
            });
